...
def init_db():
    ...
    # Migração: popula moradores a partir de unidades existentes
...
def index():
    ...
    return render_template('index.html', pendentes=pendentes, entregues_hoje=entregues_hoje,
                         unidades=unidades, pendentes_lista=pendentes_lista, ultimas=ultimas)
...
def receber():
    ...
    if primeira_unidade:
        ...
        fotos_part = "\n\nFotos do(s) pacote(s):\n" + "\n".join(fotos_links) if fotos_links else "\n\nFotos do(s) pacote(s): sem fotos"
    ...
    flash(f'{qtd} encomendas registradas com sucesso! O QR Code foi enviado ao morador via WhatsApp.', 'success')
...
def entregar():
    ...
    conn.execute('''
        UPDATE encomendas SET status = 'entregue', data_entrega = ?, entregue_por = ?
    ...
    if qtd == 1:
        flash(f'Encomenda entregue com sucesso para a unidade {unidade_label}!', 'success')
    else:
...
def api_moradores(unidade_id):
    conn = get_db()
    rows = conn.execute("SELECT * FROM moradores WHERE unidade_id = ? ORDER BY principal DESC, nome", (unidade_id,)).fetchall()
    conn.close()
...
def adicionar_unidade():
    ...
    try:
        ...
        if existe:
            ...
            conn.execute("INSERT INTO unidades (numero, telefone, nome_residente, bloco) VALUES (?, ?, ?, ?)",
...
def editar_unidade(unidade_id):
    ...
    conn = get_db()
    try:
        existe = conn.execute("SELECT id FROM unidades WHERE numero = ? AND id != ?", (numero, unidade_id)).fetchone()
        if existe:
            flash('Já existe outra unidade com esse número.', 'danger')
        else:
            conn.execute('UPDATE unidades SET numero=?, telefone=?, nome_residente=?, bloco=? WHERE id=?',
...
def excluir_unidade(unidade_id):
    conn = get_db()
    tem = conn.execute("SELECT COUNT(*) FROM encomendas WHERE unidade_id = ?", (unidade_id,)).fetchone()[0]
    if tem > 0:
...
def importar_unidades():
    ...
    if request.method == 'POST':
        ...
        if acao == 'preview':
            ...
            if not preview:
                flash('Nenhum contato válido encontrado.', 'warning')
            ...
            for i in indices:
                ...
                try:
                    existente = conn.execute("SELECT id FROM unidades WHERE numero = ?", (numero,)).fetchone()
                    if existente:
                        conn.execute('UPDATE unidades SET nome_residente=?, bloco=?, telefone=? WHERE id=?',
